...
export default function StatsSection({ term = "course", location }: { term?: string; location?: string }) {
  ...
  const isDiploma = term === 'diploma' || location === 'diploma';
    ...
    <section className="py-12 md:py-16 bg-slate-50 relative overflow-hidden">
      <div className="max-w-6xl mx-auto px-4 md:px-8 relative z-10">
        <h2 className="text-center font-semibold text-2xl md:text-[28px] text-[#0f172a] mb-10 animate-fade-in-up uppercase tracking-tight leading-tight">
          {isDiploma ? (
            <>
              <span className="text-[#b52727]">#1 Diploma in Digital Marketing Course</span>
              <br className="md:hidden" /> | 100% Placement Assistance
            </>
          ) : (
            <>
              <span className="text-[#b52727]">100% Placement Assistance</span>
              <br className="md:hidden" /> | #1 Digital Marketing {termCap} {location === 'gurgaon' ? 'in Gurugram' : ''}
            </>
          )}
        </h2>
