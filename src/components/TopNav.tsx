...
export default function TopNav({ location }: { location?: string }) {
  ...
  return (
    <nav className="bg-white py-4 px-4 md:px-12 flex justify-center items-center border-b border-gray-100 shadow-sm">
      {isGurgaon || isSouthDelhi ? (
        <a href={`tel:${isSouthDelhi ? '9266159115' : '9310076503'}`} className="bg-[#b52727] hover:bg-[#911f1f] text-white font-semibold py-2.5 px-4 sm:px-8 rounded-xl shadow-lg flex items-center space-x-2 sm:space-x-3 transition-all hover:-translate-y-0.5 active:scale-95 text-xs sm:text-sm">
          <span className="uppercase tracking-[0.08em] sm:tracking-[0.15em]">{isSouthDelhi ? '9266159115' : '9310076503'}</span>
